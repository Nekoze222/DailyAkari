...
# === バケツリレー学習 ===
Q={}
alpha=0.2; gamma=0.9; epsilon=0.3; episodes=1600
...
for ep in range(episodes):
    ...
    for step in range(25):
        ...
        lights[action]=2
        history_actions.append((tuple(lights.flatten()), action))
        if check_clear(lights): break

    # バケツリレー報酬分配
    reward = 100 if check_clear(lights) else 0
    for i, (state, action) in enumerate(reversed(history_actions)):
        if state not in Q: Q[state]={}
        Q[state][action]=Q[state].get(action,0)+alpha*(reward - Q[state].get(action,0))
        reward *= gamma  # 前の行動には少しずつ減衰報酬
